...
- **A PR that changes any probability, range, or tripwire magnitude must update the "Estimates last changed" marker in the header to the new version number.** The document version increments for every change, including wording and tooling; the estimates marker exists specifically so a reader citing a number can tell whether two versions actually differ numerically, since most don't. `check.py` prints the live count of versions published since the marker, so this file doesn't state a specific number that would go stale on the next commit. That count is versions in *publish order*, not a span of version numbers — v1.56 was reserved and never shipped, so it doesn't count and isn't a gap to explain.
