...
**Version 1.18 — 17 September 2026**
...
**Caution.** These are midpoint comparisons on overlapping ranges. Only the extinction ladder and the disempowerment alignment move exceed the width of the cells they are drawn from.
...
**The series must be named.** METR's TH1.1 reports doubling times of 188 days all-time, 129 days from 2023, and 89 days from 2024 — the same metric differing by more than 2×, so an unpinned trigger fires or not depending on which fit is used. These tripwires pin **TH1.1, 50% horizon, 2023-onward**, currently ~129 days. Deceleration sits at 260 days (2× baseline); acceleration at 75 days, 1.7× down rather than a clean half deliberately, since it must sit below the fastest existing window (89 days) so a series switch cannot fire it. Re-center if METR revises methodology again.
