...
**Version 1.20 — 17 September 2026**
...
**Automating this needs two different kinds of physical competence, and current robotics lacks both for different reasons.** One is *depth*: diagnosing an unexplained yield excursion, tracing a contamination event, identifying an unlogged vibration source. Narrow, expert, tacit, specific to one plant's quirks. The other is *breadth*: clearing a blocked drain, dealing with snow load, repairing weather damage to external plant, working outdoors in conditions that vary without warning. Individually each task is unskilled by comparison, which is exactly why it is easy to leave out of an automation estimate — but the competence required is general rather than specialised, and in unstructured outdoor settings rather than the structured indoor ones where current systems perform best. Warehouse pick-and-place is the easy case on both axes. Neither of these is.

**Plant and site infrastructure is a serial dependency in its own right.** Cleanroom qualification, ultrapure water commissioning, gas system certification and HVAC balancing all happen *after* construction and *before* production, and none of them parallelizes with what it depends on. For a leading-edge fab this is not ancillary: air handling to ISO Class 1 in lithography areas is among the largest energy loads in the plant, temperature must hold to fractions of a degree because thermal drift moves lithographic alignment, and water consumption runs to millions of gallons a day with treatment facilities that are substantial plants themselves.
...
Manhattan took roughly three years and wartime industrial mobilization three to four, and these bound *human-coordinated* projects rather than AI-coordinated ones. Warp Speed does not belong in that set: announced May 2020, first authorizations that December, it ran about a year — but it did so by having political will suspend institutional friction outright, not by compressing fab-scale construction, and it delivered a product built on scientific and platform foundations that substantially existed, even though the mRNA manufacturing base itself — lipid nanoparticle production, large-scale synthesis — was built out under the same compressed timeline. A system coordinating logistics continuously could compress much of the first category. It cannot shorten a statutory comment period, clear a multi-year EUV order backlog, or make a silicon boule grow faster. **A note on reading this section.** The 5–10% figure is dominated by serial dependence and phase-transition limits, so adding a further serial constraint does not move it — which shows the estimate is not constraint-limited at that margin, not that the constraint is slack. **A reader holding 20–30% for 2030 should revise down on this section**, because a tighter estimate has less room to absorb a new constraint than a conservative one does.
...
*Alignment moves the numbers more than capability does, in both columns.* Holding capability fixed and solving alignment moves disempowerment about 31 points (47.5% → 16% at midpoints) and extinction about 20. Holding alignment unsolved and removing the physical loop moves disempowerment 2.5 points and extinction 13.5. Physical autonomy is what makes extinction live; alignment failure is what makes losing control likely — and alignment is the larger lever on both. That holds for this contrast specifically. Run capability across the *full* ladder to no-RSI and extinction also moves about 20 points (25% → 5%), so the claim is not that capability is a minor factor, only that alignment is at least as large a lever where the two can be compared directly.
...
**On tripwire 16, and why it carries a hard-takeoff update when type 3 alone does not.** Tripwires 3 and 4 cover self-improvement by mechanism — a system-originated training change, or continual learning shipping. Frozen-weight strategy-layer improvement fires neither, since it modifies no training procedure and updates no weights, so tripwire 16 is phrased on the observable effect instead.

The apparent tension with the type-3 claim above resolves like this. Type 3 confined to algorithm engineering is ceiling-bounded and therefore does not move hard takeoff. But strategy-layer search aimed at *training methods* is no longer extracting from a fixed ceiling — it is searching for ways to raise one, which converts the loop into type 1 while keeping type 3's cheap, unclocked iteration. That combination is the thing worth watching: a fast search layer pointed at the slow expensive step. Hence the +5.
...
**No process commentary in the body, in either direction.** Not what an earlier version said, and not what a future one should do. A reader arrives with no history and no interest in one. If a gap is worth flagging for later, it belongs in an issue; if it is worth fixing, fix it.
