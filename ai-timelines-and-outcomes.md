...
**Version 1.37 — 19 September 2026**
...
**How "prefer 2b" and "2b is weaker" fit together**, since they read as contradictory: the dispute is about the *level* of uplift (4× or 1.1×), while 2b reads only the *rate* at which that level doubles. Constant bias in the level cancels out of a doubling time, so the rate can be informative while the level is unusable — hence preferring 2b on rate. It is weaker because bias that *grows* does not cancel — a plausible mechanism is self-report bias scaling with model complexity (contamination, scaffolding) rather than staying flat, which would fire 2b on rising bias rather than a real inflection. That is why 2b is scoped to disagree-with-1/2 cases, not left to stand alone. So: prefer 2b when 1/2 and 2b disagree about how fast the trend is bending; never use it for a claim about how capable systems currently are.
