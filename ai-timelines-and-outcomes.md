...
**Version 1.35 — 17 September 2026**
...
**What these estimates are conditional on.** They assume continued investment and no successful coordinated slowdown, consistent with the governance section, but do *not* assume maximum technically feasible speed. The AI Futures Project — having found its own team split on the question — now states that its forecasts *are* so conditioned, on the grounds that pricing in unknown slowdowns is less informative. A defensible and different choice; their figures read as an upper bound relative to these.
...
**On how wide these ranges should be.** Three AI Futures forecasters, using one shared model and the same evidence, put the automated-coder median at November 2027, January 2029 and January 2030 — a two-year spread arising from adjustments for unknown model limitations and data bottlenecks, not from disagreement about the model. Careful forecasters on identical inputs diverge by more than the width of most cells below. Read these ranges as narrower than the real uncertainty.
...
**A fourth possibility: alignment may bottleneck capability rather than follow it.** Brendan Halstead's argument: if epistemics and alignment gate automated coding, and are *gross complements* to narrow technical capability that cannot improve as fast, the uplift trend slows once systems become alignment-bottlenecked. Partly priced in already, since alignment problems have reduced observed uplift. But if complementarity is strong, the loop flattens for a reason no capability metric shows — and since the outcome table finds alignment dominating disempowerment risk, alignment would then gate both timeline and outcome, from independent directions.
...
**This has independent corroboration with a magnitude.** The AI Futures Project's August 2026 revision explicitly simulates the leading model's training run to account for retraining time during takeoff. It raised the median automated-coder-to-superintelligence gap from 1.22 to 1.72 years under one author's parameters and 3.86 to 4.56 under another's, and **slowed the fastest takeoffs while leaving slow ones unchanged** — the clock acting where this document claims: on hard takeoff, not on whether the loop closes.
...
**A note on reading this section.** The 5–10% figure was already set by whichever constraint binds hardest, so a newly identified serial constraint does not lower it further — it is not the *marginal* constraint. That differs from the constraint being unimportant, and from the estimate being loose. A looser estimate has more room for a new binding constraint to move it; a tight one is near its floor.
...
**Rented human labor is a physical-action channel requiring no robotics.** RentAHuman.ai (February 2026) sells human physical presence to agents via REST API and MCP server, paid in stablecoins — ID-requiring pickups, in-person attendance, hardware setup, document signing. Security researchers characterise such marketplaces as an operational primitive analogous to CAPTCHA-solving services but with physical-world reach: recruitment friction for an attack's physical components drops toward an API call. Registration figures are self-reported and likely inflated, but the primitive is demonstrated and cheap to rebuild. This affects pathway A and, as noted below, pathway B. It does **not** bear on physical RSI, where the defining requirement is a loop that needs no humans at all — renting hands is the opposite of that.
...
| 2b | **Coding-uplift doubling time** — the doubling of (uplift − 1), baseline ~3.5–5 months — falls below ~2 months or exceeds ~10 months, sustained | A second, more direct read on the loop than time horizon | Same direction and magnitude as 1 and 2; where 2b and 1/2 disagree, prefer 2b and record the conflict |
...
**Time horizon is a contested instrument, hence a second anchor.** The AI Futures Project, who pioneered using METR time horizons for forecasting, downgraded it in August 2026: it is unclear what value corresponds to a capability milestone — possibly none does — and forecasters disagree sharply about superexponentiality in ways producing vastly different predictions. They now weight **coding uplift** most heavily. The first objection misses here, since these tripwires use time horizon to detect whether the trend is *bending*, not to map a milestone; the second partly supports the design, since tripwire 1 tests bending directly. But uplift reads the loop more directly, hence 2b. The trade is measurability, and it is worse than "plausibly biased." Anthropic's internal survey gives a 4× geometric mean for present-day coding uplift; METR's controlled study measured **1.04–1.2×** — a three- to fourfold gap on the same quantity, unresolved, with METR judging its own figures biased down by selection effects and lab surveys the obvious candidate for upward bias. 2b therefore tracks the *doubling time*, not the level, which survives constant multiplicative bias but not growing bias. Until the gap narrows, 2b is the weaker instrument despite measuring the more relevant quantity.
...
**The Earth-resource scenario is parochial.** Waste-heat arguments require wanting maximal computation *here*, but Earth is a gravity well with an atmosphere impeding radiative cooling, while orbital space offers more material and better dissipation — which undercuts resource-competition mechanisms, not deliberate-hostility ones.
...
**The objection is survivorship.** Nuclear war's absence may not show competent management: it was averted on at least half a dozen occasions by individuals overriding orders or instruments. If those observations generate the apparent decline, the decline may be luck — and reading it otherwise is what a surviving observer would conclude either way. This removes the argument's best empirical support without refuting it.
...
**No process commentary in the body, in either direction** — not what an earlier version said, nor what a future one should do. A reader arrives with no history and no interest in one. A gap worth flagging belongs in an issue; a gap worth fixing gets fixed. Narrow exception: identifying metadata for continuity (a former title, a changelog pointer), which states a fact about the artifact rather than commenting on a past argument.
